...
public class TapestrySecurityIntegrationTest extends AbstractContainerTest {
	...
	protected void assertLoginPage() {
		assertNotNull(page.getElementById("tynamoLogin"),
				"Page doesn't contain login field. Not a login page.");
	...
	protected void assertSuccessInvoke() {
		assertTrue(getText("result").contains(Invoker.SUCCESS_SUFFIX),
				"Method invocation unsuccessfully");
	}
	...
	@Test(groups = { "loggedIn" }, dependsOnMethods = { "testLogin" })
	public void testComponentMethodInterceptorRequiresPermissionsILACUnauthorized()
			throws Exception {
	...
	public void testHasAllPermissionsComponentSuccess() throws Exception {
		openBase();
		assertText("HasAllPermissionsComponentSuccess",
	...
	public void testHasAllRolesComponentSuccessAlternateDivisor() throws Exception {
		openBase();
		assertText("HasAllRolesComponentSuccessAlternateDivisor",
	...
	public void testHasAnyPermissionsComponentSuccess() throws Exception {
		openBase();
		assertText("HasAnyPermissionsComponentSuccess",
	...
	public void testHasAnyRolesComponentSuccessAlternateDivisor() throws Exception {
		openBase();
		assertText("HasAnyRolesComponentSuccessAlternateDivisor",
	...
	public void testIfGrantedAllPermissionsComponentSuccess() throws Exception {
		openBase();
		assertText("IfGrantedAllPermissionsComponentSuccess",
	...
	public void testIfGrantedAllRolesComponentSuccess() throws Exception {
		openBase();
		assertText("IfGrantedAllRolesComponentSuccess",
	...
	public void testIfGrantedAllRolesComponentSuccessAlternateDivisor() throws Exception {
		openBase();
		assertText("IfGrantedAllRolesComponentSuccessAlternateDivisor",
	...
	public void testIfGrantedAnyPermissionsComponentSuccess() throws Exception {
		openBase();
		assertText("IfGrantedAnyPermissionsComponentSuccess",
	...
	public void testIfGrantedAnyRolesComponentSuccess() throws Exception {
		openBase();
		assertText("IfGrantedAnyRolesComponentSuccess",
	...
	public void testIfGrantedAnyRolesComponentSuccessAlternateDivisor() throws Exception {
		openBase();
		assertText("IfGrantedAnyRolesComponentSuccessAlternateDivisor",
	...
	public void testInterceptComponentMethodWithAjaxDeny() throws Exception {
		...
		// this executes window.location.replace so we have to wait for it. is
		// there an event we could listen instead?
	...
	public void testLocalizedUserFilterDeny() throws Exception {
		// this test can fail either if acl rules don't correctly handle path
		// matching with the requested locale,
		// or that resulting login page is not correctly localized anymore
		// (requested locale is lost)
	...
	public void testNoContextPathHandling() throws Exception {
		// without security, should actually give you the index page since
		// there's no 'user' page
		openPage("user");
		assertLoginPage();
		openPage("user#test");
		assertLoginPage();
	...
	// the following test *does not* work because of deficiency in htmlunit
	// itself. The request parameters however are saved
	// see
	// http://old.nabble.com/Problem-with-WebRequestSettings.getRequestParameters%28%29-td20167941.html
	// htmlunit's current implementation only returns what's added with
	...
	// assertLoginPage();
	// loginAction();
	// assertTrue(getLocation().startsWith(BASEURI + "about"),
	// "Request wasn't redirected to the remebered url");
	// List<NameValuePair> valuePairs =
	// page.getWebResponse().getRequestSettings().getRequestParameters();
	// assertTrue(valuePairs.contains(new NameValuePair("test", "now")),
	// "Request parameters weren't remebered");
	// }
	...
	public void testSaveRequestAnnotationHandler() throws Exception {
		...
		loginAction();
		assertTrue(getLocation().startsWith(BASEURI + "about"),
				"Request wasn't redirected to the remembered url");
	}
	...
	public void testSaveRequestFilter() throws Exception {
		...
		assertLoginPage();
		loginAction();
		assertTrue(getLocation().startsWith(BASEURI + "authc/cabinet"),
				"Request wasn't redirected to the remebered url");
	}
	...
	public void testSaveRequestWithFallbackUri() throws Exception {
		...
		assertLoginPage();
		loginAction();
		assertTrue(getLocation().startsWith(BASEURI + "index"),
				"Request wasn't redirected to the default success url");
	...
	public void testUserFilterWithAjaxDeny() throws Exception {
		...
		// Clicking on this link should make an ajax request, but HTMLUnit
		// doesn't like it and sends a non-ajax request
		...
		assertTrue(ajaxRedirectUrl.contains(APP_CONTEXT), "The ajax redirect response '"
				+ ajaxRedirectUrl + "' did not contain app context '" + APP_CONTEXT + "'");
