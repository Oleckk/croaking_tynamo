...
import de.webtech2.croaking.TapestrySecurityIntegrationTest;
import de.webtech2.croaking.services.ilac.TestModule;
import de.webtech2.croaking.services.impl.AlphaServiceImpl;
import de.webtech2.croaking.services.impl.BetaServiceImpl;
...
public class AppModule {
	...
	public static void setupSecurity(Configuration<SecurityFilterChain> configuration,
		...
		// DefaultSecurityManager defaultManager = (DefaultSecurityManager)
		// securityManager;
		//
		...
		configuration.add(factory.createChain("/roles/user/**").add(factory.roles(), "user")
				.build());
		configuration.add(factory.createChain("/roles/manager/**").add(factory.roles(), "manager")
				.build());
		configuration.add(factory.createChain("/perms/view/**").add(factory.perms(), "news:view")
				.build());
		configuration.add(factory.createChain("/perms/edit/**").add(factory.perms(), "news:edit")
				.build());

		configuration.add(factory.createChain("/ports/ssl").add(factory.ssl()).build());

		configuration.add(factory.createChain("/ports/portinuse")
				.add(factory.port(), String.valueOf(TapestrySecurityIntegrationTest.port)).build());
		configuration.add(factory.createChain("/ports/port9090").add(factory.port(), "9090")
				.build());
	...
	public void testCallingSecureOperationInternally(AlphaService alphaService) {
		// This is a secure operation but you should be able to call it when
		// Subject is not bound
		// without a security check. If this fails, it'll cause the whole app
		// startup to fail
		alphaService.invoke();
	}
